...
class Request(object):
    ...
    def __init__(
        self, event_dict: Dict[str, Any], lambda_context: Optional[Any] = None
    ) -> None:
...
class Response(object):
    ...
    def _b64encode_body_if_needed(
        ...
        if _matches_content_type(content_type, binary_types):
            if (
                _matches_content_type(content_type, ["application/json"])
                or not content_type
            ):
...
class DecoratorAPI(object):
    ...
    def schedule(
        self,
        expression: Union[str, "ScheduleExpression"],
        name: Optional[str] = None,
        description: str = "",
    ) -> Callable[..., Any]:
        return self._create_registration_function(
    ...
    def _create_registration_function(
        ...
        def _register_handler(user_handler: UserHandlerFuncType) -> Callable[..., Any]:
            ...
            self._register_handler(
                handler_type, handler_name, user_handler, wrapped, kwargs
            )
            return wrapped
...
class _HandlerRegistration(object):
    ...
    def _register_on_s3_event(
        self, name: str, handler_string: str, kwargs: Any, **unused: Dict[str, Any]
    ) -> None:
        ...
        **unused: Dict[str, Any]
        ...
        if not isinstance(route_kwargs["content_types"], list):
            raise ValueError(
                ...
                % (
                    name,
                    type(route_kwargs["content_types"]),
...
class Chalice(_HandlerRegistration, DecoratorAPI):
    ...
    def register_blueprint(
        ...
        blueprint.register(
            self, options={"name_prefix": name_prefix, "url_prefix": url_prefix}
        )
    ...
    def _register_handler(
        ...
        self._do_register_handler(
            handler_type, name, user_handler, wrapped_handler, kwargs, options
        )
    ...
    def _get_middleware_handlers(self, event_type: str) -> Any:
        ...
        return (
            func
            for func, filter_type in self.middleware_handlers
            if filter_type in [event_type, "all"]
        )
...
class ChaliceAuthorizer(object):
    def __init__(
        self, name: str, func: Callable[..., Any], scopes: Optional[List[str]] = None
    ) -> None:
        self.name: str = name
        self.func: Callable[
    ...
    def _transform_event(self, event: Dict[str, Any]) -> "AuthRequest":
        return AuthRequest(
            event["type"], event["authorizationToken"], event["methodArn"]
        )
...
class WebsocketConnectConfig(BaseEventSourceConfig):
    ...
    def __init__(
        self, name: str, handler_string: str, user_handler: UserHandlerFuncType
    ):
...
class PureLambdaWrapper(object):
    def __init__(
        self, original_func: Callable[[Dict[str, Any], Optional[Dict[str, Any]]], Any]
    ):
...
class WebsocketEventSourceHandler(EventSourceHandler):
    ...
    def __init__(
        ...
        super(WebsocketEventSourceHandler, self).__init__(
            func, event_class, middleware_handlers
        )
...
class RestAPIEventHandler(BaseLambdaHandler):
    ...
    def _main_rest_api_handler(self, event: Any, context: Any) -> Response:
        ...
        function_args = {
            name: event["pathParameters"][name] for name in route_entry.view_args
        }
    ...
    def _validate_binary_response(
        ...
        response_is_binary = _matches_content_type(
            response_content_type, self.api.binary_types
        )
    ...
    def _add_cors_headers(
        self, response: Response, cors_headers: Dict[str, str]
    ) -> None:
...
class BaseLambdaEvent(object):
    def __init__(
        self, event_dict: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> None:
...
class Blueprint(DecoratorAPI):
    def __init__(self, import_name: str) -> None:
        ...
        self._deferred_registrations: List[
            Callable[[Chalice, Dict[str, Any]], None]
        ] = []
    ...
    def _register_handler(
        ...
        def _register_blueprint_handler(app: Chalice, options: Dict[Any, Any]) -> None:
            if handler_type in _EVENT_CLASSES:
                ...
                wrapped_handler.middleware_handlers = app._get_middleware_handlers(
